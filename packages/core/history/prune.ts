...
export function shouldPrune(item: HistoryItem, now: number = Date.now()): boolean {
  if (item.clip.expiresAt && item.clip.expiresAt < now) return true;
  if (item.syncedAt < now - ONE_YEAR_MS) return true;
  return false;
}

export function pruneHistoryItems(items: HistoryItem[], now: number = Date.now()): HistoryItem[] {
